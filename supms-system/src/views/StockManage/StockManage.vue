<template>
    <div class="stockManage">
        ...
        <el-card class="box-card">
            ...
            <!-- 面板内容 -->
            <el-table
                ref="stockManageTable"
                :data="stockManageTable"
                tooltip-effect="dark"
                style="width: 100%"
            ...
            <!-- 编辑弹出的模态框 -->
            <el-dialog title="编辑库存商品" :visible.sync="flag" label-width="100px">
                <!-- 回填表单 -->
                <el-form :model="editForm" :rules="rules" label-width="60px">
                    <el-form-item label="库存：" prop="stock">
                        <el-input v-model="editForm.stock" oninput="value=value.replace(/[^\d]/g,'')" maxLength='13'></el-input>
                    </el-form-item>
                    <el-form-item label="已售：" prop="sold" >
                        <el-input v-model="editForm.sold" oninput="value=value.replace(/[^\d]/g,'')" maxLength='13'></el-input>
                    </el-form-item>
                </el-form>
                <!-- 表单尾部 -->
...
export default {
    ...
    methods: {
        ...
        getgoodsListByPage () {
            ...
            // 发送ajax请求，把每页数据发送给后端
            this.req.get('/stock/stockManage', params)
             .then(response => {
                // 接收后端返回的数据总条数 total 和 对应页码的数据 data
                let {total, data} = response;
                // 赋值给对应的变量
                this.total = total;
                this.stockManageTable = data;
                // 如果当前页没有数据 且 排除第一页
        ...
        handleEdit (id) {
            ...
            this.editId = id;
            // 发送请求，发送id给后端
            this.req.get(`/stock/stockedit`, {id})
             .then(response => {
                //  接收后端返回的数据，后端返回的数据即使只有一条 也是数组
                let result = response[0];
                // 回填表单
                this.editForm.stock = result.stock;
        ...
        saveEdit (){
            ...
            this.req.post("/stock/stockeditsave", params)
             .then(response => {
                 // 接收错误码和提示信息
